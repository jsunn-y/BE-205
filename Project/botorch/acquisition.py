...
# get rid of extra params here and above
    def get_next_query(self, domain, samp_x, samp_y, model, disc_X, double=False, verbose=True, batch_size=1, mf=False, fid=None, index=0):
        ...
        '''
        result = self.next_query(self.acq_fn, self.domain, samp_x, samp_y, model, self.xi, self.lr, self.num_iter, n_restarts=self.num_restarts, double=double, disc_X=disc_X, verbose=verbose, batch_size=batch_size, index=index)
        return result
...
# #old version of thompson sampling
...
#     #dimension is the dimesnion of the embedding
...
#         #linear kernel is a simple matrix multiplication
#         res = torch.mm(x, w.t())
#         return res
...
#     pred = []
#     k = X.size()[0]
#     #evaluate on gpu in batch to speed up (only passes embeddings through gpu), honestly doesn't seem to be faster
...
#new version of thompson sampling with botorch
def thompson_sampling(X, samp_x, samp_y, gp, xi=None, batch=1000):
    gp = gp.cpu()
    ...
    if gp.dkl:
        ...
        if gpu:
            gp = gp.cuda()
            inputs = gp.train_inputs[0].cuda()
        else:
            inputs = gp.train_inputs[0]
        gp.train_inputs = (gp.embedding(inputs),)


    #print(gp.train_inputs.shape)
    ...
    def max_obj(x):
        # could put more samples here and just add all up? or maybe just take worst as lower bound of sorts?
        # linear kernel is a simple matrix multiplication
        res = acquisition_function.forward(x)
        return res

    X = X.reshape((X.shape[0], 1, X.shape[1]))

    ## no batching (generally slower)
    # start = time.time()
    # if gp.dkl:
    #     X = gp.embedding(X)
    # # evaluate all at once (hyperthreading on cpu)
    # pred = max_obj(X)
    # print('time: ')
...
def acq_optimize_discrete(acq, bounds, samp_x, samp_y, model, xi, lr, num_iter, disc_X, n_restarts=10, min_dist=.01, double=False, verbose=True, epsilon=.001, batch_size=1, index=0):
    '''
    ...
    #looks like batch is all of the normalized X values
    batch = disc_X.float()
    if double:
        batch = batch.double()
    ...
    #loops through to find the closest point if the best one is already taken
